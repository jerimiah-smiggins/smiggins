...
function generateChangesHTML(since: "all" | string): string {
  ...
  if (since === "all") {
    ...
    for (const ver of queuedVersions) {
      ...
      output += `<h2>v${ver}</h2>${data.description}${_getVersionSpotlight(data.major_changes)}${_getChanges(ver, data.changes)}`;
    }
  } else {
    ...
    for (const ver of queuedVersions) {
      let data: versionData = changes[ver];
      output += `<h2>v${ver}</h2>${data.description}${_getChanges(ver, data.changes)}`;
    }
  }
