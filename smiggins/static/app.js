...
let _kbReverse = {};
const keybinds = {
    newPost: { defaultKey: "n", name: "Enter Post Box", description: "Lets you send a post from anywhere", callback: () => { createPostModal(); } },
    ...
    topOfTimeline: { defaultKey: "/", name: "Jump to Top", description: "Jumps to the top of the current page", callback: () => { window.scrollTo(0, 0); } },
    navModifier: { defaultKey: "g", name: "Navigation Modifier", description: "The key that needs to be pressed in order to use the other navigation keybinds", callback: () => { navModKeyPressed = true; }, releaseCallback: () => { navModKeyPressed = false; } },
    navAdmin: { defaultKey: "a", modifiers: ["nav"], name: "Admin Page", callback: () => { } },
    navHome: { defaultKey: "h", modifiers: ["nav"], name: "Home", callback: () => { if (currentPage === "home") {
            return;
        } history.pushState("home", "", "/"); renderPage("home"); } },
    navMessages: { defaultKey: "m", modifiers: ["nav"], name: "Messages", callback: () => { } },
    navNotifications: { defaultKey: "n", modifiers: ["nav"], name: "Notifications", callback: () => { if (currentPage === "notifications") {
            return;
        } history.pushState("notifications", "", "/notifications/"); renderPage("notifications"); } },
    navProfile: { defaultKey: "p", modifiers: ["nav"], name: "Your Profile", callback: () => { if (currentPage === "user" && getUsernameFromPath() === username) {
            return;
        } history.pushState("user", "", `/u/${username}/`); renderPage("user"); } },
    navSettings: { defaultKey: "s", modifiers: ["nav"], name: "Settings", callback: () => { if (currentPage === "settings") {
            return;
...
const RE_IPs = "25[0-5]|2[0-4][0-9]|1?[0-9]{1,2}";
const RE_PORTS = ":(?:6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[0-5][0-9]{4}|[1-9][0-9]{0,3})";
let urlRegex = new RegExp(`(?:https?:\\/\\/)?(?:[-a-z0-9]+\\.){1,}(?:${RE_IPs}|${RE_TLDs})\\b(?:${RE_PORTS}\\b)?(?:/(?:&amp;|[-a-zA-Z0-9@:%_\\+.~#?/=&])*)?`, "g");
let mentionRegex = new RegExp(`@([a-zA-Z0-9_\\-]{1,${limits.username}})\\b`, "g");
let hashtagRegex = /#([a-zA-Z0-9_]{1,64})\b/g;
...
function createPostModal(type, id) {
    ...
    let el = getSnippet("compose-modal", extraVars);
...
function modifyKeybindModal(kbId) {
    ...
    let el = getSnippet("keybind-modal", {
...
function postModalCreatePost(e) {
    ...
    if (postModalFor && postModalFor.type === "edit") {
        fetch("/api/post", {
            ...
            .then((ab) => {
            var _a;
            let success = !((new Uint8Array(ab)[0] >> 7) & 1);
...
function resetNotificationIndicators() {
    let notif = document.querySelector("#navbar a[data-internal-link=\"notifications\"]");
    let msg = document.querySelector("#navbar a[data-internal-link=\"messages\"]");
    let folreq = document.querySelector("#navbar a[data-internal-link=\"follow-requests\"]");
    if (notif) {
...
function _extractPost(data) {
    ...
    if (_extractBool(flags, 5)) {
        if (!_extractBool(flags, 4)) {
            ...
            let quoteIsComment = _extractBool(flags, 1);
...
function parseResponse(data, extraVariableSometimesUsed) {
    var _a;
    ...
    switch (u8arr[0]) {
        ...
        case ResponseCodes.SignUp:
            ...
            location.href = "/";
            break;
        case ResponseCodes.Follow:
            updateFollowButton(true, Boolean(u8arr[1] & (1 << 7)));
            break;
        case ResponseCodes.Unfollow:
            updateFollowButton(false);
            break;
        case ResponseCodes.Block:
            updateBlockButton(true);
            break;
        case ResponseCodes.Unblock:
        ...
        case ResponseCodes.SaveProfile:
            ...
            break;
        case ResponseCodes.DeleteAccount:
            renderPage("logout");
            break;
        case ResponseCodes.ChangePassword:
            changePasswordSuccess(_toHex(u8arr.slice(1)));
            break;
        case ResponseCodes.DefaultVisibility: break;
        case ResponseCodes.VerifyFollowers: break;
        case ResponseCodes.CreatePost:
            prependPostToTimeline(_extractPost(u8arr.slice(1))[0]);
            break;
        case ResponseCodes.Like: break;
        case ResponseCodes.Unlike: break;
        case ResponseCodes.Pin:
        ...
        case ResponseCodes.DeletePost:
            handlePostDelete(_extractInt(32, u8arr.slice(1)));
            break;
        case ResponseCodes.TimelineUser:
            displayName = _extractString(8, u8arr.slice(1));
            ...
            userUpdateStats(displayName[0], pronouns[0], bio[0], "#" + _toHex(bio[1].slice(0, 3)), "#" + _toHex(bio[1].slice(3, 6)), _extractBool(bio[1][10], 3) && "pending" || _extractBool(bio[1][10], 5), _extractBool(bio[1][10], 4), _extractInt(16, bio[1].slice(8)), _extractInt(16, bio[1].slice(6)), pinned);
        ...
        case ResponseCodes.TimelineHashtag:
            ...
            if (forwards) {
                if (extraVariableSometimesUsed === null || extraVariableSometimesUsed === void 0 ? void 0 : extraVariableSometimesUsed.startsWith("$")) {
...
let theme = _themeLs === "light" || _themeLs === "dark" ? _themeLs : "system";
function setTheme(th) {
    switch (th) {
        case "system":
            ...
            break;
        case "light":
            localStorage.setItem("smiggins-theme", "light");
            theme = "light";
            updateTheme(true);
            break;
        case "dark":
            localStorage.setItem("smiggins-theme", "dark");
            theme = "dark";
            updateTheme(false);
            break;
    }
...
const TL_POLLING_INTERVAL = 10000;
function hookTimeline(element, tls, activeTimeline, fakeBodyElement) {
    timelineToggles = [];
    timelines = tls;
    tlElement = element;
...
function handleForward(posts, end, expectedTlID, forceEvent = false) {
    ...
    if (end) {
        ...
        c.pendingForward = false;
        if (showNewElement) {
            showNewElement.innerText = "Refresh";
        }
    }
    if (forceEvent) {
        timelineShowNew();
...
function timelinePolling(forceEvent = false) {
    ...
    fetch(`${currentTl.url}${currentTl.url.includes("?") ? "&" : "?"}offset=${offset.upper}&forwards=true`)
        .then((response) => (response.arrayBuffer()))
        .then((ab) => (parseResponse(ab, (forceEvent ? "$" : "") + currentTlID)))
        .catch((err) => {
...
function createPost(content, cw, followersOnly, callback, extra) {
    fetch("/api/post", {
        ...
        body: buildRequest([
            ...
            [content, 16],
            [cw || "", 8],
            ...(extra && extra.poll ? [[extra.poll.length, 8], ...extra.poll.map((a) => ([a, 8]))] : []),
            ...(extra && extra.quote ? [[extra.quote, 32]] : []),
            ...(extra && extra.comment ? [[extra.comment, 32]] : [])
        ])
    }).then((response) => (response.arrayBuffer()))
        .then((ab) => {
...
function loginSubmitEvent(e) {
    ...
    fetch("/api/user/login", {
        ...
        .then(parseResponse)
        .then(() => { var _a; return (_a = e.target) === null || _a === void 0 ? void 0 : _a.removeAttribute("disabled"); })
        .catch((err) => {
        var _a;
        (_a = e.target) === null || _a === void 0 ? void 0 : _a.removeAttribute("disabled");
        createToast("Something went wrong!", String(err));
...
function _getLikeNotification(posts) {
    let users = posts.map((a, index) => {
        if (!offset.lower || a.timestamp < offset.lower) {
            offset.lower = a.timestamp;
        }
        if (!offset.upper || a.timestamp > offset.upper) {
            offset.upper = a.timestamp;
        }
    ...
    let lengthDifference = posts.length - users.length;
    let userFull;
...
function renderNotificationTimeline(posts, end, updateCache, moreElementOverride, prepend = false) {
    console.log(posts);
    clearTimelineStatuses();
    if (offset.lower === null && posts.length === 0) {
        let none = document.createElement("i");
    ...
    for (const post of posts) {
        ...
        if (nc === NotificationCodes.Like) {
            if (pendingLikes.length && pendingLikes[0].id !== post[0].id) {
                let el = _getLikeNotification(pendingLikes);
                if (read) {
            ...
            let el = getPost(insertIntoPostCache([post[0]])[0]);
            if (read) {
                el.dataset.notificationRead = "";
            }
            frag.append(el);
...
function saveProfile(e) {
    ...
    fetch("/api/user", {
        ...
        .then(parseResponse)
        .then(() => { var _a; (_a = e.target) === null || _a === void 0 ? void 0 : _a.removeAttribute("disabled"); })
        .catch((err) => {
        var _a;
        (_a = e.target) === null || _a === void 0 ? void 0 : _a.removeAttribute("disabled");
        createToast("Something went wrong!", String(err));
...
function saveVerifyFollowers() {
    ...
    fetch("/api/user/verify_followers", {
        method: "PATCH",
        body: buildRequest([[+(((_a = document.getElementById("verify-followers")) === null || _a === void 0 ? void 0 : _a.checked) || false), 8]])
    }).then((response) => (response.arrayBuffer()))
        .then(parseResponse);
}
...
function setKeybindElementData(kbId, el) {
    ...
    let output = `<div class="generic-margin-top">${escapeHTML(kbData.name)}: <code class="kb-key">${escapeHTML(modifiers + key)}</code> <button data-kb-id="${kbId}">Change</button></div>`;
...
function p_settingsCosmetic(element) {
    ...
    if (localStorage.getItem("smiggins-complex-timestamps")) {
        (_b = element.querySelector("#complex-timestamps")) === null || _b === void 0 ? void 0 : _b.setAttribute("checked", "");
    }
    if (localStorage.getItem("smiggins-expand-cws")) {
        (_c = element.querySelector("#expand-cws")) === null || _c === void 0 ? void 0 : _c.setAttribute("checked", "");
    }
    if (localStorage.getItem("smiggins-hide-interactions")) {
        (_d = element.querySelector("#hide-interactions")) === null || _d === void 0 ? void 0 : _d.setAttribute("checked", "");
    }
    if (localStorage.getItem("smiggins-auto-show-posts")) {
        (_e = element.querySelector("#auto-show")) === null || _e === void 0 ? void 0 : _e.setAttribute("checked", "");
    }
    (_f = element.querySelector("#theme")) === null || _f === void 0 ? void 0 : _f.addEventListener("change", settingsThemeSelection);
    (_g = element.querySelector("#complex-timestamps")) === null || _g === void 0 ? void 0 : _g.addEventListener("change", genericCheckbox("smiggins-complex-timestamps"));
    (_h = element.querySelector("#expand-cws")) === null || _h === void 0 ? void 0 : _h.addEventListener("change", genericCheckbox("smiggins-expand-cws"));
    (_j = element.querySelector("#hide-interactions")) === null || _j === void 0 ? void 0 : _j.addEventListener("change", genericCheckbox("smiggins-hide-interactions"));
    (_k = element.querySelector("#auto-show")) === null || _k === void 0 ? void 0 : _k.addEventListener("change", genericCheckbox("smiggins-auto-show-posts"));
...
function userUpdateStats(displayName, pronouns, bio, colorOne, colorTwo, following, blocking, numFollowing, numFollowers, pinned) {
    var _a;
    let userUsername = getUsernameFromPath();
    ...
    if (followingElement) {
        followingElement.innerText = floatintToStr(numFollowing);
    }
    if (followersElement) {
        followersElement.innerText = floatintToStr(numFollowers);
    }
}
...
function toggleFollow(e) {
    ...
    fetch("/api/user/follow", {
        ...
        .then(parseResponse)
        .then(() => { followButton.disabled = false; })
        .catch((err) => {
