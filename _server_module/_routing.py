...
def get_post_page(post_id: Union[str, int]) -> Union[tuple[flask.Response, int], flask.Response]:
    ...
    return return_dynamic_content_type(format_html(
        ...
        custom_replace={
            ...
            "{{POST_ID}}": str(post_id),
            "{{LIKED}}": "false" if logged_out else str("interactions" in post_info and "likes" in post_info["interactions"] and user_id in post_info["interactions"]["likes"]).lower(),
            ...
            "{{COMMENT}}": "false",
            "{{LOGGED_IN}}": "false" if logged_out else "true",
            "{{HTML_ESC_CONTENT}}": escape_html(post_info["content"])
        }
    ))
...
def get_comment_page(post_id: Union[str, int]) -> Union[tuple[flask.Response, int], flask.Response]:
    ...
    if int(post_id) >= generate_comment_id(inc=False) and int(post_id) <= 0:
        if logged_out:
            return return_dynamic_content_type(format_html(
                open(f"{ABSOLUTE_CONTENT_PATH}/redirect_index.html", "r").read()
            ), "text/html"), 404
